...
flowchart TD
    ...
    SB --> W[AppointmentWorker]
    W --> UC2[ProcessAppointmentUseCase]
    UC2 -->|COMPLETED| Cosmos
    UC2 --> SQL[(Azure SQL)]
    UC2 --> ACS[Notificación por Email ACS]
```

Un solo tópico, una sola suscripción, un solo worker — publish → consume de punta a punta sin piezas muertas.
...
POST /api/appointments
  → CreateAppointmentUseCase
      ...
      → Tópico Service Bus "appointment-created"
          → AppointmentWorker (una sola suscripción/worker para PE + CL)
...
**86 tests en 21 archivos de test.** Las pruebas corren completamente en memoria -- no requieren cuenta de Azure, variables de entorno, ni conexión de red.
