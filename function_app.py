...
def appointment_worker(message: func.ServiceBusMessage, context: func.Context) -> None:
    # countryISO doesn't route to a different downstream resource (single Cosmos/SQL
    # DB for both PE and CL, unlike the AWS sibling's per-country RDS instances) - it's
    # only used for log correlation, so one subscription/one worker covers both countries.
    country = message.subject or "UNKNOWN"
    _process_worker_message(message.get_body().decode("utf-8"), country, context)
