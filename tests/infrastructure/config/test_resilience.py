...
def test_with_resilience_records_one_failure_per_logical_call_despite_retries(monkeypatch):
    ...
    # Fewer failing logical calls than CB_WINDOW_SIZE, even though each one retries internally
    # RETRY_ATTEMPTS times. If retry incorrectly wrapped the breaker, this would already have
    # recorded CB_WINDOW_SIZE failures and opened the circuit.
    for _ in range(CB_WINDOW_SIZE // 2):
        with pytest.raises(ValueError):
            run(always_fails)

    # The circuit must still be closed: a plain success should go through untouched.
