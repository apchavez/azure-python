...
// Published by ServiceBusEventPublisher.publish_cancelled — no subscriber today (nothing
// consumes cancellations yet), but the topic itself must exist or publishing fails at runtime.
resource cancelledTopic 'Microsoft.ServiceBus/namespaces/topics@2022-10-01-preview' = {
  parent: sb
  name: 'appointment-cancelled'
  properties: { defaultMessageTimeToLive: 'P14D' }
}

// Single worker subscription for both countries. countryISO (set as message.Subject
// by the publisher) doesn't route to a different downstream resource here - PE and CL
// share the same Cosmos DB / Azure SQL instance, unlike the AWS sibling's per-country
// RDS instances - so there's no reason to split into per-country subscriptions/workers.
  ...
  properties: {
    ...
    siteConfig: {
      ...
      appSettings: [
        ...
        { name: 'SERVICEBUS_CANCELLED_TOPIC', value: 'appointment-cancelled' }
