...
class ProcessAppointmentUseCase:
    ...
    def execute(self, appointment_id: str) -> None:
        ...
        # Idempotency guard: if stage 2 already completed this appointment, skip silently.
        if appointment.status == AppointmentStatus.COMPLETED:
            return

        # Marked completed in-memory only (for the relational snapshot below) - Cosmos itself
        # isn't updated here; that's stage 2's (ConfirmAppointmentUseCase) job.
