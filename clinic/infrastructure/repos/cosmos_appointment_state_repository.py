...
class CosmosAppointmentStateRepository:
    ...
    def _resilient(self, fn: Any) -> Any:
        # Circuit breaker wraps retry (not the other way around): each logical operation
        # (including its internal retries) contributes exactly one success/failure record to
        # the breaker's window. Wrapping it the other way inflates the failure count by up to
        # RETRY_ATTEMPTS per real incident, tripping the breaker on transient blips far below
        # the intended threshold and stalling unrelated calls (other reads/writes) for the
        # full open-circuit cooldown.
    ...
    def _to_domain(item: dict[str, Any]) -> Appointment:
        return Appointment(
            ...
            # Normalize case: older documents (written before the status values were
            # standardized to lowercase) persisted "PENDING"/"COMPLETED" uppercase. A single
            # such legacy record would otherwise raise ValueError and 500 the entire query
            # (list, pagination, history) for whatever insuredId/appointmentId it belongs to.
